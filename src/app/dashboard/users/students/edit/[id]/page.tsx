...
export default function EditStudentPage() {
    ...
    useEffect(() => {
        if (id) {
            Promise.all([fetchStudentDetails(), fetchSkills()]);
        }
    }, [id]);
    ...
    const populateForm = (data: any) => {
        setFormData({
            ...
            educations: data.educations && data.educations.length > 0 
                ? data.educations.map((e: any) => ({ ...e, yearOfPassing: String(e.yearOfPassing) })) 
                : [{ institutionName: "", level: "University", degree: "", yearOfPassing: "" }],
            internships: data.internships && data.internships.length > 0 
                ? data.internships.map((i: any) => ({ ...i, startDate: i.startDate ? i.startDate.split('T')[0] : "", endDate: i.endDate ? i.endDate.split('T')[0] : "" })) 
        ...
        if (data.skills && Array.isArray(data.skills)) {
            ...
            selectedSkills.remove(...selectedSkills.items.map(i => i.id)); // Clear existing
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                if (!response2.ok) throw new Error("Failed to update student");
            }
        ...
        <div className="flex flex-col gap-8 max-w-3xl mx-auto w-full py-8 px-4 sm:px-6">
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                    <h1 className="text-display-sm font-semibold text-primary">Edit Student</h1>
                    <p className="text-md text-tertiary">
                        Update the student profile details below.
                    </p>
                </div>
                ...
                {currentStep === 1 && (
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <Input
                            ...
                            wrapperClassName="col-span-1 sm:col-span-2"
                            isDisabled // Email immutable usually
                        />
                        <Input
                            label="Phone"
