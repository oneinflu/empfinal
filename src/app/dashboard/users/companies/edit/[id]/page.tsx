...
export default function EditCompanyPage() {
    ...
    const fetchCompanyDetails = async () => {
        try {
            ...
            const linkedIn = data.socialLinks?.find((l: any) => l.platform === "LinkedIn")?.url || "";
            const twitter = data.socialLinks?.find((l: any) => l.platform === "Twitter")?.url || "";

            setFormData({
                name: data.user?.name || "",
                email: data.user?.email || "",
                phone: data.phone || "", // Assuming phone is available in the profile or user object
    ...
    const handleSubmit = async () => {
        try {
            ...
            // Assuming the update endpoint is PUT /admin/companies/:id or similar
            // Since we don't have explicit documentation, we'll try the most logical one
            // or we might need to use the profile update endpoint if available.
            ...
            if (!response.ok) {
                 // Fallback to another potential endpoint if 404
                 const response2 = await fetch(`${API_BASE_URL}/company-profiles/${id}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });
                if (!response2.ok) throw new Error("Failed to update company");
            }
        ...
        <div className="flex flex-col gap-8 max-w-3xl mx-auto w-full py-8 px-4 sm:px-6">
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                    <h1 className="text-display-sm font-semibold text-primary">Edit Company</h1>
                    <p className="text-md text-tertiary">
                        Update the company profile details below.
                    </p>
                </div>
            ...
            <div className="flex flex-col gap-6 bg-white rounded-xl shadow-sm border border-secondary p-6 sm:p-8">
                ...
                {currentStep === 1 && (
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <Input
                            ...
                            wrapperClassName="col-span-1 sm:col-span-2"
                            isDisabled // Email is usually immutable or handled separately
                        />
                        {/* Password removed */}
                        <Input
                            label="Phone"
                ...
                <div className="flex justify-between pt-6 mt-2 border-t border-secondary">
                    ...
                    <Button 
                        ...
                        isDisabled={isLoading}
                    >
                        {currentStep === STEPS.length ? (isLoading ? "Saving..." : "Save Changes") : "Next Step"}
                    </Button>
                </div>
            </div>
        </div>
