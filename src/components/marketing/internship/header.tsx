...
import { Building01, MarkerPin01, CheckCircle, Share01, Bookmark } from "@untitledui/icons";
import Image from "next/image";
import { Button } from "@/components/base/buttons/button";

export const InternshipHeader = ({ opportunity }: { opportunity: Opportunity }) => {
    return (
                    ...
                    <div className="flex items-center gap-5 bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-5 hover:bg-white/15 transition-colors">
                        <div className="relative w-14 h-14 flex items-center justify-center">
                            <svg className="w-full h-full transform -rotate-90">
                                ...
                                <circle cx="28" cy="28" r="24" stroke="currentColor" strokeWidth="4" fill="transparent" strokeDasharray={150.7} strokeDashoffset={150.7 * (1 - 0.85)} className="text-green-400" />
                            </svg>
                            <span className="absolute text-sm font-bold text-white">85%</span>
                        </div>
                        <div>
                            <div className="font-bold text-white text-lg">Great Match</div>
                            <div className="text-sm text-slate-300">Top 5% of applicants</div>
                        </div>
                    </div>
                </div>
            </div>
